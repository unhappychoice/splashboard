...
The preview above is a live render — `cargo xtask` composes the TOML
below through the real layout engine using each fetcher's sample data,
so what you see is exactly what the splash looks like (minus real-data
variation).
